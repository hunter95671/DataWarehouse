...
--数据装载
--2021-07-31
load data inpath '/origin_data/gmall/log/topic_log/2021-07-31' into table ods_log partition (dt = '2021-07-31');

--创建索引
--hadoop jar /opt/module/hadoop-3.1.3/share/hadoop/common/hadoop-lzo-0.4.20.jar com.hadoop.compression.lzo.DistributedLzoIndexer /warehouse/gmall/ods/ods_log/dt=2021-07-31
...
--数据装载
insert overwrite table dim_coupon_info partition (dt = '2021-07-31')
select id,
       coupon_name,
       coupon_type,
       condition_amount,
...
--数据装载
insert overwrite table dim_activity_rule_info partition (dt = '2021-07-31')
select ar.id,
       ar.activity_id,
       ai.activity_name,
       ar.activity_type,
...
select
    ...
    old_operate_time,
    old_start_date,
    cast(date_add('2021-08-01',-1) as string),
    cast(date_add('2021-08-01',-1) as string) dt
from tmp
...
--数据装载
insert overwrite table dwd_start_log partition(dt='2021-07-31')
select
...
--数据装载
insert overwrite table dwd_page_log partition(dt='2021-07-31')
select
    get_json_object(line,'$.common.ar'),
    get_json_object(line,'$.common.ba'),
    get_json_object(line,'$.common.ch'),
...
insert overwrite table dwd_action_log partition(dt='2021-07-31')
select
    get_json_object(line,'$.common.ar'),
    get_json_object(line,'$.common.ba'),
    get_json_object(line,'$.common.ch'),
...
--数据装载
insert overwrite table dwd_display_log partition(dt='2021-07-31')
select
    get_json_object(line,'$.common.ar'),
    get_json_object(line,'$.common.ba'),
    get_json_object(line,'$.common.ch'),
...
--数据装载
insert overwrite table dwd_error_log partition(dt='2021-07-31')
select
    get_json_object(line,'$.common.ar'),
    get_json_object(line,'$.common.ba'),
    get_json_object(line,'$.common.ch'),
...
--每日
--2021-08-01
insert overwrite table dwd_comment_info partition(dt='2021-08-01')
select
    id,
    user_id,
    sku_id,
    ...
    appraise,
    create_time
from ods_comment_info where dt='2021-08-01';
...
--每日
--2021-08-01
insert overwrite table dwd_order_detail partition(dt='2021-08-01')
select
    od.id,
    od.order_id,
    oi.user_id,
...
from
(
    select * from ods_order_refund_info where dt='2021-07-31'
)ri
left join
(
    select id,province_id from ods_order_info where dt='2021-07-31'
)oi
...
--每日
--2021-08-01
insert overwrite table dwd_order_refund_info partition(dt='2021-08-01')
select
    ri.id,
    ri.user_id,
    ri.order_id,
...
from
(
    select * from ods_order_refund_info where dt='2021-08-01'
)ri
left join
(
    select id,province_id from ods_order_info where dt='2021-08-01'
)oi
on ri.order_id=oi.id;
...
--数据装载
--首日
insert overwrite table dwd_cart_info partition(dt='2021-07-31')
select
    id,
    user_id,
    sku_id,
...
--每日
insert overwrite table dwd_cart_info partition(dt='2021-08-01')
select
    id,
    user_id,
    sku_id,
...
--数据装载
--首日
insert overwrite table dwd_favor_info partition(dt='2021-07-31')
select
    id,
    user_id,
    sku_id,
...
--每日
insert overwrite table dwd_favor_info partition(dt='2021-08-01')
select
    id,
    user_id,
    sku_id,
...
from
(
    select * from ods_payment_info where dt='2021-07-31'
)pi
left join
(
    select id,province_id from ods_order_info where dt='2021-07-31'
...
(
    ...
    from
    (
        select * from ods_payment_info where dt='2021-08-01'
    )pi
    left join
    (
        select id,province_id from ods_order_info where dt='2021-08-01'
...
select
    ...
    case
        ...
        when times.ts['1004'] is not null and date_add(date_format(times.ts['1004'],'yyyy-MM-dd'),7)<='2021-07-31' and times.ts['1005'] is null then date_add(date_format(times.ts['1004'],'yyyy-MM-dd'),7)
...
select
    ...
    case
        ...
        when new.finish_time is not null and date_add(date_format(new.finish_time,'yyyy-MM-dd'),7)='2021-08-01' and new.refund_time is null then '2021-08-01'
...
(
    ...
    where (dt='2021-08-01'
    or dt='9999-99-99')
    and date_format(create_time,'yyyy-MM-dd')='2021-08-01'
    group by user_id
),
tmp_pay as
(
...
(
    ...
    (
        select
            ...
            refund_num
        from dwd_order_refund_info
        where dt>=date_add('2021-08-01',-15)
    )ri
    on rp.order_id=ri.order_id
    and rp.sku_id=rp.sku_id
    group by rp.user_id
...
(
    select
        user_id,
        sum(if(date_format(get_time,'yyyy-MM-dd')='2021-08-01',1,0)) coupon_get_count,
        sum(if(date_format(using_time,'yyyy-MM-dd')='2021-08-01',1,0)) coupon_using_count,
        sum(if(date_format(used_time,'yyyy-MM-dd')='2021-08-01',1,0)) coupon_used_count
    from dwd_coupon_use
    where (dt='2021-08-01' or dt='9999-99-99')
    and (date_format(get_time, 'yyyy-MM-dd') = '2021-08-01'
    or date_format(using_time,'yyyy-MM-dd')='2021-08-01'
    or date_format(used_time,'yyyy-MM-dd')='2021-08-01')
    group by user_id
),
tmp_comment as
(
...
insert overwrite table dws_user_action_daycount partition(dt='2021-08-01')
select
    coalesce(tmp_login.user_id,tmp_cf.user_id,tmp_order.user_id,tmp_pay.user_id,tmp_ri.user_id,tmp_rp.user_id,tmp_comment.user_id,tmp_coupon.user_id,tmp_od.user_id),
    nvl(login_count,0),
    nvl(cart_count,0),
...
--数据装载
insert overwrite table dws_visitor_action_daycount partition(dt='2021-07-31')
select
    t1.mid_id,
    t1.brand,
    t1.model,
...
(
    ...
    from dwd_order_detail
    where (dt='2021-08-01'
    or dt=date_add('2021-08-01',-1))
    and order_id in
    (
        select order_id from dwd_payment_info where dt='2021-08-01'
    )
    group by sku_id
),
tmp_ri as
...
insert overwrite table dws_sku_action_daycount partition(dt='2021-08-01')
select
    sku_id,
    sum(order_count),
    sum(order_num),
...
(
    select
        coupon_id,
        sum(if(date_format(get_time,'yyyy-MM-dd')='2021-08-01',1,0)) get_count,
        sum(if(date_format(using_time,'yyyy-MM-dd')='2021-08-01',1,0)) order_count,
        sum(if(date_format(used_time,'yyyy-MM-dd')='2021-08-01',1,0)) payment_count,
        sum(if(date_format(expire_time,'yyyy-MM-dd')='2021-08-01',1,0)) expire_count
    from dwd_coupon_use
    where dt='9999-99-99'
    or dt='2021-08-01'
    group by coupon_id
...
insert overwrite table dws_coupon_info_daycount partition(dt='2021-08-01')
select
    coupon_id,
    sum(get_count),
    sum(order_count),
...
insert overwrite table dws_activity_info_daycount partition(dt='2021-08-01')
select
    activity_rule_id,
    activity_id,
    sum(order_count),
...
insert overwrite table dws_area_stats_daycount partition(dt='2021-08-01')
select
    province_id,
    sum(visit_count),
    sum(login_count),
...
--数据装载
--首日
insert overwrite table dwt_user_topic partition(dt='2021-07-31')
select
    id,
    login_date_first,--以用户的创建日期作为首次登录日期
    nvl(login_date_last,date_add('2021-07-31',-1)),--若有历史登录记录，则根据历史记录获取末次登录日期，否则统一指定一个日期
...
(
    select
        ...
        sum(if(dt='2021-07-31',login_count,0)) login_last_1d_count,
        sum(if(dt='2021-07-31' and login_count>0,1,0)) login_last_1d_day_count,
        sum(if(dt>=date_add('2021-07-31',-6),login_count,0)) login_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6) and login_count>0,1,0)) login_last_7d_day_count,
        sum(if(dt>=date_add('2021-07-31',-29),login_count,0)) login_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29) and login_count>0,1,0)) login_last_30d_day_count,
        ...
        sum(if(dt='2021-07-31',order_count,0)) order_last_1d_count,
        sum(if(dt='2021-07-31',order_activity_count,0)) order_activity_last_1d_count,
        sum(if(dt='2021-07-31',order_activity_reduce_amount,0)) order_activity_reduce_last_1d_amount,
        sum(if(dt='2021-07-31',order_coupon_count,0)) order_coupon_last_1d_count,
        sum(if(dt='2021-07-31',order_coupon_reduce_amount,0)) order_coupon_reduce_last_1d_amount,
        sum(if(dt='2021-07-31',order_original_amount,0)) order_last_1d_original_amount,
        sum(if(dt='2021-07-31',order_final_amount,0)) order_last_1d_final_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_count,0)) order_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),order_activity_count,0)) order_activity_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),order_activity_reduce_amount,0)) order_activity_reduce_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_coupon_count,0)) order_coupon_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),order_coupon_reduce_amount,0)) order_coupon_reduce_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_original_amount,0)) order_last_7d_original_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_final_amount,0)) order_last_7d_final_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_count,0)) order_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),order_activity_count,0)) order_activity_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),order_activity_reduce_amount,0)) order_activity_reduce_last_30d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_coupon_count,0)) order_coupon_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),order_coupon_reduce_amount,0)) order_coupon_reduce_last_30d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_original_amount,0)) order_last_30d_original_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_final_amount,0)) order_last_30d_final_amount,
        ...
        sum(if(dt='2021-07-31',payment_count,0)) payment_last_1d_count,
        sum(if(dt='2021-07-31',payment_amount,0)) payment_last_1d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),payment_count,0)) payment_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),payment_amount,0)) payment_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),payment_count,0)) payment_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),payment_amount,0)) payment_last_30d_amount,
        sum(payment_count) payment_count,
        sum(payment_amount) payment_amount,
        sum(if(dt='2021-07-31',refund_order_count,0)) refund_order_last_1d_count,
        sum(if(dt='2021-07-31',refund_order_num,0)) refund_order_last_1d_num,
        sum(if(dt='2021-07-31',refund_order_amount,0)) refund_order_last_1d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),refund_order_count,0)) refund_order_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),refund_order_num,0)) refund_order_last_7d_num,
        sum(if(dt>=date_add('2021-07-31',-6),refund_order_amount,0)) refund_order_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),refund_order_count,0)) refund_order_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),refund_order_num,0)) refund_order_last_30d_num,
        sum(if(dt>=date_add('2021-07-31',-29),refund_order_amount,0)) refund_order_last_30d_amount,
        ...
        sum(if(dt='2021-07-31',refund_payment_count,0)) refund_payment_last_1d_count,
        sum(if(dt='2021-07-31',refund_payment_num,0)) refund_payment_last_1d_num,
        sum(if(dt='2021-07-31',refund_payment_amount,0)) refund_payment_last_1d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),refund_payment_count,0)) refund_payment_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),refund_payment_num,0)) refund_payment_last_7d_num,
        sum(if(dt>=date_add('2021-07-31',-6),refund_payment_amount,0)) refund_payment_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),refund_payment_count,0)) refund_payment_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),refund_payment_num,0)) refund_payment_last_30d_num,
        sum(if(dt>=date_add('2021-07-31',-29),refund_payment_amount,0)) refund_payment_last_30d_amount,
        ...
        sum(if(dt='2021-07-31',cart_count,0)) cart_last_1d_count,
        sum(if(dt>=date_add('2021-07-31',-6),cart_count,0)) cart_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-29),cart_count,0)) cart_last_30d_count,
        sum(cart_count) cart_count,
        sum(if(dt='2021-07-31',favor_count,0)) favor_last_1d_count,
        sum(if(dt>=date_add('2021-07-31',-6),favor_count,0)) favor_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-29),favor_count,0)) favor_last_30d_count,
        sum(favor_count) favor_count,
        sum(if(dt='2021-07-31',coupon_get_count,0)) coupon_last_1d_get_count,
        sum(if(dt='2021-07-31',coupon_using_count,0)) coupon_last_1d_using_count,
        sum(if(dt='2021-07-31',coupon_used_count,0)) coupon_last_1d_used_count,
        sum(if(dt>=date_add('2021-07-31',-6),coupon_get_count,0)) coupon_last_7d_get_count,
        sum(if(dt>=date_add('2021-07-31',-6),coupon_using_count,0)) coupon_last_7d_using_count,
        sum(if(dt>=date_add('2021-07-31',-6),coupon_used_count,0)) coupon_last_7d_used_count,
        sum(if(dt>=date_add('2021-07-31',-29),coupon_get_count,0)) coupon_last_30d_get_count,
        sum(if(dt>=date_add('2021-07-31',-29),coupon_using_count,0)) coupon_last_30d_using_count,
        sum(if(dt>=date_add('2021-07-31',-29),coupon_used_count,0)) coupon_last_30d_used_count,
        sum(coupon_get_count) coupon_get_count,
        sum(coupon_using_count) coupon_using_count,
        sum(coupon_used_count) coupon_used_count,
        sum(if(dt='2021-07-31',appraise_good_count,0)) appraise_last_1d_good_count,
        sum(if(dt='2021-07-31',appraise_mid_count,0)) appraise_last_1d_mid_count,
        sum(if(dt='2021-07-31',appraise_bad_count,0)) appraise_last_1d_bad_count,
        sum(if(dt='2021-07-31',appraise_default_count,0)) appraise_last_1d_default_count,
        sum(if(dt>=date_add('2021-07-31',-6),appraise_good_count,0)) appraise_last_7d_good_count,
        sum(if(dt>=date_add('2021-07-31',-6),appraise_mid_count,0)) appraise_last_7d_mid_count,
        sum(if(dt>=date_add('2021-07-31',-6),appraise_bad_count,0)) appraise_last_7d_bad_count,
        sum(if(dt>=date_add('2021-07-31',-6),appraise_default_count,0)) appraise_last_7d_default_count,
        sum(if(dt>=date_add('2021-07-31',-29),appraise_good_count,0)) appraise_last_30d_good_count,
        sum(if(dt>=date_add('2021-07-31',-29),appraise_mid_count,0)) appraise_last_30d_mid_count,
        sum(if(dt>=date_add('2021-07-31',-29),appraise_bad_count,0)) appraise_last_30d_bad_count,
        sum(if(dt>=date_add('2021-07-31',-29),appraise_default_count,0)) appraise_last_30d_default_count,
...
--每日
insert overwrite table dwt_user_topic partition(dt='2021-08-01')
select
    nvl(1d_ago.user_id,old.user_id),
    nvl(old.login_date_first,'2021-08-01'),
    if(1d_ago.user_id is not null,'2021-08-01',old.login_date_last),
    ...
    if(old.order_date_first is null and 1d_ago.order_count>0, '2021-08-01', old.order_date_first),
    if(1d_ago.order_count>0,'2021-08-01',old.order_date_last),
    ...
    if(old.payment_date_first is null and 1d_ago.payment_count>0, '2021-08-01', old.payment_date_first),
    if(1d_ago.payment_count>0,'2021-08-01',old.payment_date_last),
...
(
    select
        ...
        appraise_default_count
    from dwt_user_topic
    where dt=date_add('2021-08-01',-1)
)old
full outer join
(
    select
...
(
    select
        ...
        appraise_default_count
    from dws_user_action_daycount
    where dt=date_add('2021-08-01',-7)
)7d_ago
...
(
    select
        ...
        appraise_default_count
    from dws_user_action_daycount
    where dt=date_add('2021-08-01',-30)
)30d_ago
...
--数据装载
insert overwrite table dwt_visitor_topic partition(dt='2021-07-31')
select
    ...
    case when old.mid_id is null and 1d_ago.is_new=1 then '2021-07-31'
         when old.mid_id is null and 1d_ago.is_new=0 then '2020-06-13'--无法获取准确的首次登录日期，给定一个数仓搭建日之前的日期
         else old.visit_date_first end,
    if(1d_ago.mid_id is not null,'2021-07-31',old.visit_date_last),
...
(
    ...
    where dt=date_add('2021-07-31',-1)
)old
full outer join
(
    select
...
(
    select
        ...
        visit_count
    from dws_visitor_action_daycount
    where dt=date_add('2021-07-31',-7)
)7d_ago
...
(
    select
        ...
        visit_count
    from dws_visitor_action_daycount
    where dt=date_add('2021-07-31',-30)
)30d_ago
...
--数据装载
--首日
insert overwrite table dwt_sku_topic partition(dt='2021-07-31')
select
    id,
    nvl(order_last_1d_count,0),
...
(
    select
        sku_id,
        sum(if(dt='2021-07-31',order_count,0)) order_last_1d_count,
        sum(if(dt='2021-07-31',order_num,0)) order_last_1d_num,
        sum(if(dt='2021-07-31',order_activity_count,0)) order_activity_last_1d_count,
        sum(if(dt='2021-07-31',order_coupon_count,0)) order_coupon_last_1d_count,
        sum(if(dt='2021-07-31',order_activity_reduce_amount,0)) order_activity_reduce_last_1d_amount,
        sum(if(dt='2021-07-31',order_coupon_reduce_amount,0)) order_coupon_reduce_last_1d_amount,
        ...
        sum(if(dt='2021-07-31',order_final_amount,0)) order_last_1d_final_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_count,0)) order_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),order_num,0)) order_last_7d_num,
        sum(if(dt>=date_add('2021-07-31',-6),order_activity_count,0)) order_activity_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),order_coupon_count,0)) order_coupon_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),order_activity_reduce_amount,0)) order_activity_reduce_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_coupon_reduce_amount,0)) order_coupon_reduce_last_7d_amount,
        ...
        sum(if(dt>=date_add('2021-07-31',-6),order_final_amount,0)) order_last_7d_final_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_count,0)) order_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),order_num,0)) order_last_30d_num,
        sum(if(dt>=date_add('2021-07-31',-29),order_activity_count,0)) order_activity_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),order_coupon_count,0)) order_coupon_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),order_activity_reduce_amount,0)) order_activity_reduce_last_30d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_coupon_reduce_amount,0)) order_coupon_reduce_last_30d_amount,
        ...
        sum(order_final_amount) order_final_amount,
        sum(if(dt='2021-07-31',payment_count,0)) payment_last_1d_count,
        sum(if(dt='2021-07-31',payment_num,0)) payment_last_1d_num,
        sum(if(dt='2021-07-31',payment_amount,0)) payment_last_1d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),payment_count,0)) payment_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),payment_num,0)) payment_last_7d_num,
        sum(if(dt>=date_add('2021-07-31',-6),payment_amount,0)) payment_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),payment_count,0)) payment_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),payment_num,0)) payment_last_30d_num,
        sum(if(dt>=date_add('2021-07-31',-29),payment_amount,0)) payment_last_30d_amount,
        sum(payment_count) payment_count,
...
--每日
insert overwrite table dwt_sku_topic partition(dt='2021-08-01')
select
...
--数据装载
--首日
insert overwrite table dwt_coupon_topic partition(dt='2021-07-31')
select
    id,
...
(
    select
        coupon_id coupon_id,
        sum(if(dt='2021-07-31',get_count,0)) get_last_1d_count,
        sum(if(dt>=date_add('2021-07-31',-6),get_count,0)) get_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-29),get_count,0)) get_last_30d_count,
        sum(get_count) get_count,
        sum(if(dt='2021-07-31',order_count,0)) order_last_1d_count,
        sum(if(dt='2021-07-31',order_reduce_amount,0)) order_last_1d_reduce_amount,
        sum(if(dt='2021-07-31',order_original_amount,0)) order_last_1d_original_amount,
        sum(if(dt='2021-07-31',order_final_amount,0)) order_last_1d_final_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_count,0)) order_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),order_reduce_amount,0)) order_last_7d_reduce_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_original_amount,0)) order_last_7d_original_amount,
        sum(if(dt>=date_add('2021-07-31',-6),order_final_amount,0)) order_last_7d_final_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_count,0)) order_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),order_reduce_amount,0)) order_last_30d_reduce_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_original_amount,0)) order_last_30d_original_amount,
        sum(if(dt>=date_add('2021-07-31',-29),order_final_amount,0)) order_last_30d_final_amount,
        sum(order_count) order_count,
        ...
        sum(order_final_amount) order_final_amount,
        sum(if(dt='2021-07-31',payment_count,0)) payment_last_1d_count,
        sum(if(dt='2021-07-31',payment_reduce_amount,0)) payment_last_1d_reduce_amount,
        sum(if(dt='2021-07-31',payment_amount,0)) payment_last_1d_amount,
        sum(if(dt>=date_add('2021-07-31',-6),payment_count,0)) payment_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),payment_reduce_amount,0)) payment_last_7d_reduce_amount,
        sum(if(dt>=date_add('2021-07-31',-6),payment_amount,0)) payment_last_7d_amount,
        sum(if(dt>=date_add('2021-07-31',-29),payment_count,0)) payment_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),payment_reduce_amount,0)) payment_last_30d_reduce_amount,
        sum(if(dt>=date_add('2021-07-31',-29),payment_amount,0)) payment_last_30d_amount,
        sum(payment_count) payment_count,
        sum(payment_reduce_amount) payment_reduce_amount,
        sum(payment_amount) payment_amount,
        sum(if(dt='2021-07-31',expire_count,0)) expire_last_1d_count,
        sum(if(dt>=date_add('2021-07-31',-6),expire_count,0)) expire_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-29),expire_count,0)) expire_last_30d_count,
...
--每日
insert overwrite table dwt_coupon_topic partition(dt='2021-08-01')
select
...
--数据装载
--首日
insert overwrite table dwt_activity_topic partition(dt='2021-07-31')
select
    t1.activity_rule_id,
    t1.activity_id,
    nvl(order_last_1d_count,0),
...
--每日
insert overwrite table dwt_activity_topic partition(dt='2021-08-01')
select
...
--数据装载
--首日
insert overwrite table dwt_area_topic partition(dt='2021-07-31')
select
    id,
...
(
    select
        ...
        sum(if(dt='2021-07-31',visit_count,0)) visit_last_1d_count,
        sum(if(dt='2021-07-31',login_count,0)) login_last_1d_count,
        sum(if(dt>=date_add('2021-07-31',-6),visit_count,0)) visit_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-6),login_count,0)) login_last_7d_count,
        sum(if(dt>=date_add('2021-07-31',-29),visit_count,0)) visit_last_30d_count,
        sum(if(dt>=date_add('2021-07-31',-29),login_count,0)) login_last_30d_count,
...
--每日
insert overwrite table dwt_area_topic partition(dt='2021-08-01')
select
    ...
    (
        ...
        (
            select
                ...
                ts,
                recent_days,
                if(visit_date_first>=date_add('2021-07-31',-recent_days+1),'1','0') is_new
            from
            (
                select
                    t1.mid_id,
            ...
            where dt>=date_add('2021-07-31',-recent_days+1)
        )t4
    )t5
...
(
    ...
    (
        ...
        (
            ...
            where dt>=date_add('2021-07-31',-30)
            and dt>=date_add('2021-07-31',-recent_days+1)
        )t2
    )t3
)t4
...
(
    select
        ...
        if(recent_days=0,'1970-01-01',date_add('2021-07-31',-recent_days+1)) recent_days_ago
...
(
    ...
    from dwt_user_topic
    where dt='2021-07-31'
    and login_date_last=date_add('2021-07-31',-7)
)churn
join
(
    select
    ...
    (
        ...
        from dwt_user_topic
        where dt='2021-07-31'
        and login_date_last='2021-07-31'
    )t1
    join
    (
        select
...
(
    ...
    (
        ...
        (
            ...
            where dt>=date_add('2021-07-31',-29)
...
select
    ...
    datediff('2021-07-31',login_date_first) retention_day,
    sum(if(login_date_last='2021-07-31',1,0)) retention_count,
    count(*) new_user_count,
    cast(sum(if(login_date_last='2021-07-31',1,0))/count(*)*100 as decimal(16,2)) retention_rate
from dwt_user_topic
where dt='2021-07-31'
and login_date_first>=date_add('2021-07-31',-7)
and login_date_first<'2021-07-31'
...
(
    ...
    from dim_coupon_info
    where dt='2021-07-31'
    and date_format(start_time,'yyyy-MM-dd')>=date_add('2021-07-31',-29)
)t1
left join
(
    select
